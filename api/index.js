    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            for (const nome of nomes) {
                const { data: novoJogador } = await supabase
                    .from('jogadores')
    ...
    try {
        ...
        if (supabase) {
            ...
            const novasVitorias = acao === 'add' 
                ? jogadorData.vitorias + 1 
                : acao === 'remove' && jogadorData.vitorias > 0 
                    ? jogadorData.vitorias - 1 
                    : jogadorData.vitorias;
    ...
    try {
        ...
        if (supabase) {
            ...
            const novosPontos = acao === 'add' 
                ? jogadorData.pontos + 1 
                : acao === 'remove' && jogadorData.pontos > 0 
                    ? jogadorData.pontos - 1 
                    : jogadorData.pontos;
...
app.post('/api/jogadores/reset', async (req, res) => {
    try {
        ...
        res.json({ mensagem: 'Dados resetados com sucesso' });
    } catch (err) {
        res.status(500).json({ erro: err.message });
    }
});
